...
function setUsuario(login, nome, senha, email, status, usuariotipo, datacadastro, res){
    ...
    cliente.query(('insert into usuarios ("login", "nome", "senha", "email", "status", "usuariotipo", "datacadastro") values ('+"'"+login+"', '"+nome+"', '"+ hash + "', '"+email+"', '"+status+"', '"+usuariotipo+"', '" +datacadastro+"');"), (err, result)=> {
        if(err){
